...
def get_test_images(infer_dir, infer_img, infer_list=None):
    ...
    if infer_list:
        ...
        for ext in exts:
            print('Format: {}/**/*.{}'.format(infer_dir, ext))
...
def run(FLAGS, cfg):
    ...
    if FLAGS.do_eval:
        ...
        images = FLAGS.infer_dir

        print(f"images from the start: {images}")
...
if __name__ == '__main__':
    print("INFER MODEL IN PROGRESS...")
    sys.stdout = open("logging/infer_out.txt", "w")
    sys.stderr = open("logging/infer_err.txt", "w")
