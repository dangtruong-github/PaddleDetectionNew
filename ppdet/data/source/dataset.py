...
class ImageFolder(DetDataset):
    def __init__(self,
        ...
        print(f"self.image_dir init: {self.image_dir}")
    ...
    def _parse(self):
        ...
        print(f"self.image_dir inside _parses: {self.image_dir}")
        if not isinstance(image_dir, Sequence):
            image_dir = [image_dir]
        images = []
        print(f"image_dir: {image_dir}")
        print(f"list_dir image_dir: {len(os.listdir(image_dir))}")
        for im_dir in os.listdir(image_dir):
            # if im_dir[-4:] != ".jpg":
            #    continue
            im_cur_dir = os.path.join(image_dir, im_dir)
            print(im_cur_dir)
            if os.path.isdir(im_cur_dir):
                print(f"image path 1: {im_dir}")
                images.extend(_make_dataset(im_cur_dir))
            elif os.path.isfile(im_cur_dir) and _is_valid_file(im_cur_dir):
                images.append(im_cur_dir)
                print(f"image path 2: {im_dir}")
            else:
                pass
                #print(f"image path 3: {im_dir}")
